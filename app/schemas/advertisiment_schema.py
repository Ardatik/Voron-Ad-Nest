...
class AdvertisimentCreate(BaseModel):
    title: str = Field(..., min_length=1, max_length=200, description="Заголовок рекламы")
...
class AdvertisimentUpdate(BaseModel):
    title: Optional[str] = Field(None, min_length=1, max_length=200, description="Заголовок рекламы")
    ...
    @model_validator(mode="after")
    def validate_dates(self):
        if self.date_start_adv and self.date_finish_adv and self.date_start_adv >= self.date_finish_adv:
            raise ValueError("Дата окончания рекламы должна быть позже даты начала")
        return self
