...
class UserRegister(BaseModel):
    ...
    def validate_date_of_birth(cls, birth_date: date) -> date:
        ...
        if (date.today() - birth_date).days / 365 > 100:
            raise ValueError("указана корректуню дату рождения")
        return birth_date
...
class ChangeAccountData(BaseModel):
    ...
    first_name: Optional[str] = Field(
        default=None, min_length=2, max_length=16, description="Имя пользователя"
    )
    last_name: Optional[str] = Field(
        default=None, min_length=2, max_length=20, description="Фамилия пользователя"
    )
    patronymic_name: Optional[str] = Field(
        default=None, min_length=2, max_length=20, description="Отчество пользователя"
    )
    date_of_birth: Optional[date] = Field(
        default=None, description="Дата рождения пользователя"
    )
    password: Optional[str] = Field(
        default=None, min_length=8, max_length=20, description="Пароль пользователя"
    )
